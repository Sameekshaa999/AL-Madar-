...
const PropertiesDeep = () => {
  ...
  // if (JSON.stringify(searchDict) === JSON.stringify(defaultSearchDict)) {
  ...
  function RenderSearch() {
    ...
    searchData = qatarPropertiesData
      .map((item) => Search(item))
      .filter(Boolean);
  ...
  function PropertySlide(props) {
    const forceUpdate = RenderSearch();
    return (
      <>
        ...
        <div className="h-[42rem] sm:h-[50rem] md:h-[36rem] lg:h-[31rem] font-montserrat">
          ...
          >
            {value
              ? [qatarPropertiesData[0]].map((item) => (
                  <SwiperSlide id={item.id} onClick={forceUpdate}>
